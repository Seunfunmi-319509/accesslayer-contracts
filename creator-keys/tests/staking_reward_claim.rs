...
fn test_claim_stake_reward_fails_while_lock_is_active() {
    ...
    let position = client
        ...
        .unwrap();
    let unlock_ledger = position.unlock_ledger;
...
fn test_claim_stake_reward_pays_out_and_unlocks_after_lock_period() {
    ...
    let result = client.claim_stake_reward(&creator, &holder, &stake_id);
    assert_eq!(result.reward, pool_after_buy);
...
fn test_claim_stake_reward_splits_pool_pro_rata_across_stakers() {
    ...
    let stake_id_a = client.stake_keys_locked(&creator, &holder_a, &3u32, &STAKE_LOCK_LEDGERS);
    let stake_id_b = client.stake_keys_locked(&creator, &holder_b, &1u32, &STAKE_LOCK_LEDGERS);
    ...
    let result_a = client.claim_stake_reward(&creator, &holder_a, &stake_id_a);
    assert_eq!(result_a.reward, expected_a);
    assert_eq!(client.get_total_staked(&creator), 1);

    let result_b = client.claim_stake_reward(&creator, &holder_b, &stake_id_b);
    ...
    assert_eq!(result_b.reward, expected_b_before_a_claims);
    assert_eq!(client.get_total_staked(&creator), 0);
    assert_eq!(client.get_staking_rewards_pool(&creator), 0);
}
